      ...
      <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-8 p-6 sm:p-8">
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs uppercase text-gray-400 font-bold tracking-widest mt-4">{product?.category || "SUV"}</p>
            <h1 className="text-3xl font-extrabold text-slate-900 mt-1">{product?.name || "Hyundai Tucson"}</h1>
            
            <div className="mt-4 inline-flex items-baseline gap-1 bg-slate-900 text-white px-4 py-2 rounded-xl">
              <span className="text-2xl font-bold">${product?.pricePerDay || "110"}</span>
            ...
            <div className="mt-6 space-y-3 border-t border-gray-100 pt-4 text-sm text-gray-700">
              ...
              <div className="flex items-center gap-2"><span>👥 Capacity:</span> <span className="font-semibold">{product?.seats || "5"} seats</span></div>
            </div>
          </div>
